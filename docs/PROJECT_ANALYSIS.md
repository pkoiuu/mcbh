...
| UpdateService | 153 | GitHub Releases 更新检查 | 10s 超时；国内镜像 api.gitproxy.dev |
...
- `download-build.ps1`：镜像（api.gitproxy.dev > ghfast.top > 直连）下载 CI 的 `latest/baihe-build.zip`，解压部署到 `src/Baihe.Host/bin/CI/...` 并启动（本地体验 CI 构建）。
