...
ExitSetupTitle=退出安装
ExitSetupMessage=安装尚未完成。如果现在退出，程序将不会被安装。%n%n您可以稍后再次运行安装程序以完成安装。%n%n确定退出安装吗？
...
SelectTasksLabel2=选择安装 [name] 时希望安装程序执行的附加任务，然后点击"下一步"。
...
FinishedLabel=安装程序已在您的计算机上完成 [name] 的安装。可以通过安装的快捷方式启动应用程序。
...
UninstalledAll=%1 已成功地从您的计算机上移除。
