...
namespace Baihe.Core.SourceGenerators;
...
public sealed class ConfigGenerator : IIncrementalGenerator
{
    ...
    private static object? _GetItemCandidate(GeneratorSyntaxContext ctx)
    {
        ...
        var attrDefItem = compilation.GetTypeByMetadataName("Baihe.Core.App.Configuration.ConfigItemAttribute`1");
        var attrDefAny  = compilation.GetTypeByMetadataName("Baihe.Core.App.Configuration.AnyConfigItemAttribute`1");
    ...
    private static object? _GetGroupCandidate(GeneratorSyntaxContext ctx)
    {
        ...
        var compilation = ctx.SemanticModel.Compilation;
        var attrDef = compilation.GetTypeByMetadataName("Baihe.Core.App.Configuration.ConfigGroupAttribute");
    ...
    private static object? _GetRegisterConfigEventCandidate(GeneratorSyntaxContext ctx)
    {
        ...
        var compilation = ctx.SemanticModel.Compilation;
        var attrDef = compilation.GetTypeByMetadataName("Baihe.Core.App.Configuration.RegisterConfigEventAttribute");
        if (attrDef is null) return null;
    ...
    private static string _GenerateAdditionalSource(ConfigTree tree)
    {
        ...
        sb.AppendLine("using Baihe.Core.App.Configuration;");
        sb.AppendLine();
        ...
        IReadOnlyDictionary<INamedTypeSymbol, GroupModel> groupLookup)
    {
        ...
        sb.AppendLine("// 此文件由 Source Generator 自动生成，请勿手动修改");
        sb.AppendLine();
        sb.AppendLine("namespace Baihe.Core.App.Configuration;");
        sb.AppendLine();
    ...
    public static string _RenderSourceCode(SemanticModel sm, ExpressionSyntax expr)
    {
        ...
        if (sym is IFieldSymbol fs && fs.ContainingType?.ToDisplayString() == "Baihe.Core.App.Configuration.ConfigSource")
        {
