...
public static class SharedExtensions
{
    ...
    {
        const string argConfigPrefix = "Baihe.Core.App.Configuration.ArgConfig<";
        var isArgConfig = typeName.StartsWith(argConfigPrefix);
        if (isArgConfig)
        {
            fullTypeName = typeName;
            // 提取 ArgConfig<TValue> 中的 TValue 部分
            // 动态计算偏移量，避免硬编码导致的错误
            typeName = typeName.Substring(argConfigPrefix.Length, typeName.Length - argConfigPrefix.Length - 1);
        }
