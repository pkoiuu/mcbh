...
///         它面向 C# 代码侧调用；前端 UI 层（如 Svelte）应通过 IPC 或序列化方式获取本地化文本。
///     </p>
///     <p>
///         文本资源通过 <see cref="RegisterTexts" /> 注册的字典查找；未注册的键在调试构建返回 <c>!key!</c>，
///         发布构建返回 <paramref name="key" /> 本身。
///     </p>
///     <p>
...
{
    private static readonly Dictionary<string, string> _Resources = new(StringComparer.Ordinal);
    ...
    /// <summary>
    ///     注册本地化文本资源。后续调用会覆盖同名的键。
    /// </summary>
    /// <param name="texts">
    ///     本地化文本字典，键为资源键，值为本地化文本。
    /// </param>
    public static void RegisterTexts(IDictionary<string, string> texts)
    {
    ...
    public static string Text(string key)
    {
        ...
        if (_Resources.TryGetValue(key, out var text)) return text;
